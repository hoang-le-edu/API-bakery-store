...
import {
    ...
    fetchWards,
    resetPaymentLink,
} from "../../redux/action/paymentAction.js";
...
const PaymentDetailPopup = ({ isVisible, index, cart, refetchData }) => {
    ...
    const [form, setForm] = useState({
        ...
        paymentMethod: "Cash",
        branch: "",
        voucher_code: "", // Changed from voucher (UUID) to voucher_code (string)
        voucher_shipping_code: "", // Changed from voucher_shipping (UUID) to voucher_shipping_code (string)
    });
    ...
    const chooseVoucher = (voucherId) => {
        setForm({
            ...form,
            voucher: form.voucher === voucherId ? "" : voucherId,
        });
    };
    ...
    const chooseVoucherShipping = (voucherId) => {
        setForm({
            ...form,
            voucher_shipping:
                form.voucher_shipping === voucherId ? "" : voucherId,
        });
    };

    const applyVoucher = () => {
        if (form.voucher !== "") {
            const voucher = vouchers.discount.find(
                (v) => v.id === form.voucher,
            );
            if (voucher.discount_type === "percent") {
                let value = (cart.total_price * voucher.discount_percent) / 100;
                setDiscountAmount(
                    value > voucher.limit_per_order
                        ? voucher.limit_per_order
                        : value,
                );
            } else {
        ...
        if (form.voucher_shipping !== "") {
            const voucher = vouchers.shipping_fee.find(
                (v) => v.id === form.voucher_shipping,
            );
            if (voucher.discount_type === "percent") {
                let value = (cart.total_price * voucher.discount_percent) / 100;
                setDeliveryDiscount(
    ...
    const proceedOrder = async () => {
        try {
            ...
            // Build order data - customer inputs voucher CODE, server validates and calculates
            const orderData = {
                order_detail_ids: cart.order_details.map((detail) => detail.id),
                receiver_name: form.receiverName,
                receiver_address: `${form.street}, ${form.ward}, ${form.district}, ${form.province}`,
                payment_method: form.paymentMethod,
                voucher_code: form.voucher_code || undefined, // Send voucher CODE
                voucher_shipping_code: form.voucher_shipping_code || undefined, // Send shipping voucher CODE
                note: form.note,
                ...
                shipping_fee: deliveryAmount,
                // Note: discount_number removed - server calculates it from voucher codes
            };
            ...
            if (response.status === 200) {
                // Extract server-calculated discounts from response
                const serverDiscounts = response.data.data?.discounts;
                ...
                if (serverDiscounts) {
                    ...
                    // Update local state with server values for confirmation display
                    setDiscountAmount(serverDiscounts.product_discount || 0);
                    setDeliveryDiscount(serverDiscounts.shipping_discount || 0);
                }
                ...
                notify(
                    "error",
                    "An error occurred while proceeding with the order. Please try again.",
                );
            }
            ...
            // Handle voucher validation errors from server
            if (error.response?.status === 422) {
                const errorData = error.response.data;

                // Check for voucher-specific errors
                if (errorData.error) {
                    notify("error", `Voucher Error: ${errorData.error}`);
    ...
    const getCheckoutUrl = async () => {
        try {
            ...
            notify(
                "error",
                "An error occurred while creating payment link. Please try again.",
            );
        }
    };
            ...
            {showSuccess ? (
                <div className="flex flex-col h-full w-full justify-center items-center mt-55">
                    ...
                    <span className="text-xl mt-4 flex flex-col items-center">
                        {socketStatus
                            ? t("DETAIL_PAYMENT.PAY_SUCCESS")
                            : t("DETAIL_PAYMENT.PLACE_ORDER_SUCCESS")}
                    ...
                    {showButtons && (
                        <div className="flex gap-2 pt-4">
                            ...
                            {form.paymentMethod === "Banking" &&
                                showPaynowButton && (
                                    <button
                                        className="bg-[#ABBA7C] text-white px-4 py-2 rounded-full font-bold"
                                        onClick={getCheckoutUrl}
                                    >
                                        {loading ? (
                                            <SpinnerLoading />
                                        ) : (
                                            t("DETAIL_PAYMENT.PAY_NOW")
                                        )}
                                    </button>
                                )}
                        </div>
                ...
                <section
                    className={`relative bg-gray-100 ${isVisible ? "block" : "hidden"}`}
                >
                                    ...
                                    <div className="space-y-1 p-5 pt-4">
                                        <div className="flex w-full justify-between">
                                            <div className="flex gap-1 items-center">
                                                ...
                                                <label className="font-bold text-[#f26d78] dark:text-white">
                                                    {t(
                                                        "DETAIL_PAYMENT.DELIVERY_ADDRESS",
                                                    )}
                                                </label>
                                            </div>
                                        ...
                                        <div className="mt-4">
                                            <div className="grid grid-cols-2 gap-2">
                                                ...
                                                <div>
                                                    <label className="block mb-2 text-sm font-medium text-gray-500 dark:text-white">
                                                        {t(
                                                            "DETAIL_PAYMENT.DELIVERY_ADDRESSES.PROVINCE",
                                                        )}
                                                    </label>
                                                    <select
                                                        value={form.province}
                                                    ...
                                                    >
                                                        <option value="">
                                                            {t(
                                                                "DETAIL_PAYMENT.DELIVERY_ADDRESSES.SELECT_PROVINCE",
                                                            )}
                                                        </option>
                                                        {provinces?.map(
                                                            (province) => (
                                                ...
                                                <div>
                                                    <label className="block mb-2 text-sm font-medium text-gray-500 dark:text-white">
                                                        {t(
                                                            "DETAIL_PAYMENT.DELIVERY_ADDRESSES.DISTRICT",
                                                        )}
                                                    </label>
                                                    <select
                                                        value={form.district}
                                                    ...
                                                    >
                                                        <option value="">
                                                            {t(
                                                                "DETAIL_PAYMENT.DELIVERY_ADDRESSES.SELECT_DISTRICT",
                                                            )}
                                                        </option>
                                                        {districts
                                                            ?.filter(
                                                                (d) =>
                                                                    d.ProvinceID.toString() ===
                                                                    form.province.toString(),
                                                            )
                                                            .map((district) => (
                                                                <option
                                                                    key={
                                            ...
                                            <div className="grid grid-cols-1 gap-2">
                                                {/* Select Ward */}
                                                <div>
                                                    <label className="block mb-2 text-sm font-medium text-gray-500 dark:text-white mt-2">
                                                        {t(
                                                            "DETAIL_PAYMENT.DELIVERY_ADDRESSES.WARD",
                                                        )}
                                                    </label>
                                                    <select
                                                        value={form.ward}
                                                    ...
                                                    >
                                                        <option value="">
                                                            {t(
                                                                "DETAIL_PAYMENT.DELIVERY_ADDRESSES.SELECT_WARD",
                                                            )}
                                                        </option>
                                                        {wards?.map((ward) => (
                                                            <option
                                                ...
                                                <div>
                                                    <label className="block mb-2 text-sm font-medium text-gray-500 dark:text-white">
                                                        {t(
                                                            "DETAIL_PAYMENT.DELIVERY_ADDRESSES.STREET",
                                                        )}
                                                    </label>
                                                    <input
                                                        type="text"
                                                        ...
                                                        className="bg-white border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                                        placeholder={t(
                                                            "DETAIL_PAYMENT.DELIVERY_ADDRESSES.PLACEHOLDER_STREET",
                                                        )}
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 gap-2">
                                                <div>
                                                    ...
                                                    <input
                                                        ...
                                                        className="bg-white border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                                        placeholder={t(
                                                            "DETAIL_PAYMENT.PLACEHOLDER_RECEIVER_NAME",
                                                        )}
                                                    />
                                                </div>
                                                <div>
                                                    ...
                                                    <input
                                                        ...
                                                        className="bg-white border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                                        placeholder={t(
                                                            "DETAIL_PAYMENT.PLACEHOLDER_PHONE_NUMBER",
                                                        )}
                                                    />
                                                </div>
                                                <div>
                                                ...
                                                <div>
                                                    ...
                                                    >
                                                        <option value="Cash">
                                                            {t(
                                                                "DETAIL_PAYMENT.PAYMENT_METHODS.COD",
                                                            )}
                                                        </option>
                                                        <option value="Banking">
                                                            {t(
                                                                "DETAIL_PAYMENT.PAYMENT_METHODS.ONLINE",
                                                            )}
                                                        </option>
                                                    </select>
                                                </div>
                            ...
                            <div className="flex justify-center gap-4 mt-4">
                                ...
                                >
                                    {isProcessing ? (
                                        ...
                                        t("DETAIL_PAYMENT.PROCEED_ORDER")
                                    )}
                                </button>
                            </div>
                        </div>
                                                        ...
                                                        >
                                                            ...
                                                            <div>
                                                                <input
                                                                    type="checkbox"
                                                                    checked={
                                                                        form.voucher_shipping ===
                                                                        voucher.id
                                                                    }
                                                                    onChange={() =>
                                                                        chooseVoucherShipping(
                                                                            voucher.id,
                                                                        )
                                                                    }
                                                                />
